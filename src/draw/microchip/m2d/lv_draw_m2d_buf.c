...
static lv_draw_buf_t * m2d_create(const lv_draw_buf_handlers_t * handlers, uint32_t w, uint32_t h, lv_color_format_t cf, uint32_t stride);
...
lv_draw_m2d_buf_t * lv_draw_m2d_search_buf_map(const lv_image_dsc_t * img_dsc)
{
    ...
    uint32_t height = h->h;
    size_t stride = m2d_get_stride(m2d_buf);
...
struct m2d_buffer * lv_draw_m2d_buf_get_tmp(lv_draw_m2d_buf_t * draw_m2d_buf)
{
    struct m2d_buffer * buf = draw_m2d_buf->m2d_tmp;

    if(!buf) {
        const lv_image_header_t * h = &draw_m2d_buf->base_buf.header;
        /* Force the pixel format to ARGB8888 to allow alpha blending. */
        buf = m2d_alloc(h->w, h->h, M2D_PF_ARGB8888, h->w * sizeof(uint32_t));
        draw_m2d_buf->m2d_tmp = buf;
    }
                                  ...
                                  uint32_t stride)
{
    ...
    if(draw_m2d_buf == NULL) {
        LV_LOG_ERROR("failed to allocated memory for lv_draw_m2d_buf_t");
        goto out;
    }
