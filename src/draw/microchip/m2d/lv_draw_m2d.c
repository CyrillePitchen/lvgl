...
static int32_t m2d_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task)
{
    ...
    switch(task->type) {
        case LV_DRAW_TASK_TYPE_FILL:
            claim = lv_draw_m2d_fill_is_supported((lv_draw_fill_dsc_t *)task->draw_dsc);
            break;
        case LV_DRAW_TASK_TYPE_IMAGE:
            claim = lv_draw_m2d_image_is_supported((lv_draw_image_dsc_t *)task->draw_dsc);
            break;

        case LV_DRAW_TASK_TYPE_LAYER:
            claim = lv_draw_m2d_layer_is_supported((lv_draw_image_dsc_t *)task->draw_dsc);
            break;
...
static int32_t m2d_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    ...
    draw_m2d_unit->task_act->state = LV_DRAW_TASK_STATE_READY;
