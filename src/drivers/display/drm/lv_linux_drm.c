...
void lv_linux_drm_set_file(lv_display_t * disp, const char * file, int64_t connector_id)
{
    ...
    if (handlers && handlers->import_cb) {
        ...
        if(!buf0) {
            LV_LOG_ERROR("Failed to import DRM buffer 0: resolution=%" LV_PRIu32 "x%" LV_PRIu32 ", format=0x%02" LV_PRIx32 ", stride=%" LV_PRIu32 ", DRM PRIME fd=%" LV_PRId32,
        ...
        if(!buf1) {
            LV_LOG_ERROR("Failed to import DRM buffer 1: resolution=%" LV_PRIu32 "x%" LV_PRIu32 ", format=0x%02" LV_PRIx32 ", stride=%" LV_PRIu32 ", DRM PRIME fd=%" LV_PRId32,
                         dsc.w, dsc.h, dsc.cf, dsc.stride, dsc.fd);
