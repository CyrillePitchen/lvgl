...
struct _lv_map_entry_t {
    lv_link_t link;
    lv_map_t * map;
    struct _lv_map_entry_t * parent;
    struct _lv_map_entry_t * children[2];
    lv_map_key_t * key;
    lv_map_value_t * value;
    bool color;
};
...
static lv_map_entry_t * lv_map_entry_create(lv_map_t * map, const lv_map_key_t * key, const lv_map_value_t * value)
{
    ...
    entry->value = (lv_map_value_t *)((lv_uintptr_t)entry + LV_MAP_ALIGN_UP_TO_PTR(sizeof(lv_map_entry_t)) + LV_MAP_ALIGN_UP_TO_PTR(class_p->key_size));
